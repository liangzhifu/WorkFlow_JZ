		...
		try{
			...
			if(timeTaskList == null){
				...
				for(int i = 0; i < timeTaskList.size(); i++){
					index = index % 11;
					sendMailDao = null;
					sendMailDao = MailSenderFactory.getSender("404_4M@cn.joysonsafety.com", "", "10.235.12.102", "25");
					...
					try{
						...
						sendMailDao.send("404_4M@cn.joysonsafety.com", toMailAddress, subject, content);
						timeTask.setDeleteState(1);
						this.timeTaskDao.deleteTimeTask(timeTask);
					}catch(Exception e1){
						e1.printStackTrace();
